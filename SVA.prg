...
{	
	...
	ELSE		
		...
		ACC(AxisIndex) = Axis_Acc(GetPLC_AxisNumber(AxisIndex)) * 5
		DEC(AxisIndex) = Axis_Acc(GetPLC_AxisNumber(AxisIndex)) * 5
		JERK(AxisIndex) = Axis_Acc(GetPLC_AxisNumber(AxisIndex)) * 30
	END
...
INT CheckAxisLimit(INT AxisIndex);
VOID PLC_RunMotin(INT AxisIndex);
VOID ProcessAxisCommands(INT axis, INT plcIdx);


WHILE 1

	IF CurrentAxis <> 1 | CurrentAxis <> 3 | CurrentAxis <> 7 | CurrentAxis <> 17 | CurrentAxis <> 21

		ProcessAxisCommands(CurrentAxis, GetPLC_AxisNumber(CurrentAxis))
!		SCSCSC=GetPLC_AxisNumber(CurrentAxis)
!		DISP CurrentAxis,SCSCSC
	END
...
{	
	INT prog
	prog = GetAxisHomeBuffer(axis)
	IF plcIdx < 0
		RET 
	END
	IF axis=7|axis=17|axis=21
...
!! enable/disable motor
	IF EDGE(Axis_Control(plcIdx).0 = 1) & ^MST(axis).#ENABLED
		ENABLE axis;		DISP "ENABLE ", axis,plcIdx,prog
	END
	IF EDGE(Axis_Control(plcIdx).1 = 1) & MST(axis).#ENABLED
		DISABLE axis;		DISP "DISABLE ", axis,plcIdx,prog
	END

!! HOME
	IF EDGE(Axis_Control(plcIdx).2 = 1)
		WAIT 200

		IF ^PST(prog).#RUN
			START prog, 1
			DISP "HOME ", axis,plcIdx,prog
		ELSE			
			STOP prog;			START prog, 1
		END
		DISP "HOME ", axis,plcIdx,prog
	END

!! Jog 
	IF Axis_Control(plcIdx).3 = 1 & Axis_Control(plcIdx).7 <> 1 & ^MST(axis).#MOVE !P
		JOGMove(axis) = 1;
		WAIT 10;
		CheckAxisPara(axis);
		JOG axis,+ 
	ELSEIF Axis_Control(plcIdx).3 = 0 & JOGMove(axis) = 1
		JOGMove(axis) = 0;		KILL axis
	END
	IF Axis_Control(plcIdx).4 = 1 & Axis_Control(plcIdx).7 <> 1 & ^MST(axis).#MOVE !N
		JOGMove(axis) = 2;
		WAIT 10;
		CheckAxisPara(axis);
		JOG axis,- 
	ELSEIF Axis_Control(plcIdx).4 = 0 & JOGMove(axis) = 2
		JOGMove(axis) = 0;		KILL axis
	END

!! ABS/INC
	IF EDGE(Axis_Control(plcIdx).5 = 1) & Axis_Control(plcIdx).7 <> 1 & ^MST(axis).#MOVE  !ABS
		WAIT 10;		CheckAxisPara(axis);		PLC_RunMotin(axis)
	END
	IF EDGE(Axis_Control(plcIdx).6 = 1) & Axis_Control(plcIdx).7 <> 1 & ^MST(axis).#MOVE  !INC
		WAIT 10;		CheckAxisPara(axis);		PTP/R axis, Axis_Pos(plcIdx)
	END

!! KILL
	IF EDGE(Axis_Control(plcIdx).7 = 1)
		KILLALL
		DISP "KILLALL"
	END
...
{	
	...
	IF ^MST(AxisIndex).#ENABLED
		ENABLE AxisIndex
		DISP AxisIndex, ":PLC NOT ENABLE"
	END
	IF Axis_Vel(AxisIndex) <= 0 | Axis_Vel(AxisIndex) > XVEL(AxisIndex)
		VEL(AxisIndex) = AxisVel(AxisIndex)
		ACC(AxisIndex) = AxisVel(AxisIndex) * 5
		DEC(AxisIndex) = AxisVel(AxisIndex) * 5
		JERK(AxisIndex) = AxisVel(AxisIndex) * 30
...
VOID PLC_AxisRunning(INT AxisIndex1, INT AxisIndex2 =- 1, INT AxisIndex3 =- 1, INT AxisIndex4 =- 1, INT AxisIndex5 =- 1, INT AxisIndex6 =- 1, INT AxisIndex7 =- 1, INT AxisIndex8 =- 1)
{	

	AxisMoveFlage(AxisIndex1) = 1
	AxisMoveTime(AxisIndex1) = Axis_Pos(GetPLC_AxisNumber(AxisIndex1)) / VEL(AxisIndex1) * 2

	IF AxisIndex2 <>- 1
		AxisMoveFlage(AxisIndex2) = 1
		AxisMoveTime(AxisIndex2) = Axis_Pos(GetPLC_AxisNumber(AxisIndex2)) / VEL(AxisIndex2) * 2

	END
	
	IF AxisIndex3 <>- 1
		AxisMoveFlage(AxisIndex3) = 1
		AxisMoveTime(AxisIndex3) = Axis_Pos(GetPLC_AxisNumber(AxisIndex3)) / VEL(AxisIndex3) * 2

	END
	
	IF AxisIndex4 <>- 1
		AxisMoveFlage(AxisIndex4) = 1
		AxisMoveTime(AxisIndex4) = Axis_Pos(GetPLC_AxisNumber(AxisIndex4)) / VEL(AxisIndex4) * 2

	END
	
	IF AxisIndex5 <>- 1
		AxisMoveFlage(AxisIndex5) = 1
		AxisMoveTime(AxisIndex5) = Axis_Pos(GetPLC_AxisNumber(AxisIndex5)) / VEL(AxisIndex5) * 2

	END
	
	IF AxisIndex6 <>- 1
		AxisMoveFlage(AxisIndex6) = 1
		AxisMoveTime(AxisIndex6) = Axis_Pos(GetPLC_AxisNumber(AxisIndex6)) / VEL(AxisIndex6) * 2

	END
	
	IF AxisIndex7 <>- 1
		AxisMoveFlage(AxisIndex7) = 1
		AxisMoveTime(AxisIndex7) = Axis_Pos(GetPLC_AxisNumber(AxisIndex7)) / VEL(AxisIndex7) * 2

	END
	
	IF AxisIndex8 <>- 1
		AxisMoveFlage(AxisIndex8) = 1
		AxisMoveTime(AxisIndex8) = Axis_Pos(GetPLC_AxisNumber(AxisIndex8)) / VEL(AxisIndex8) * 2
...
!PNAME=
!PDESC=
!ON PA_ComSupMotionType=PCOccurAlarm & PST(8).#RUN<>1 & (PA_AlarmLevel=Alarm_High | PA_AlarmLevel=Alarm_Normal | PA_AlarmLevel=Alarm_Tips)
!  IF PA_AlarmCode=0
!      PA_AlarmCode=AlarmCode_PCOccurAlarm
!   END
!
!RET
...
ACC(BT_Y0)=PA_HomeVel(BT_Y0)*10
DEC(BT_Y0)=PA_HomeVel(BT_Y0)*10
JERK(BT_Y0)=PA_HomeVel(BT_Y0)*50

	IF ^MFLAGS(BT_Y0).#BRUSHOK             !commutation
		COMMUT (BT_Y0), XRMS(BT_Y0)*0.8      !commut=commutation
		TILL MFLAGS(BT_Y0).#BRUSHOK
	END

HomeAxis(BT_Y0)=BT_Y0
HomeStartTime(BT_Y0)=TIME
HOME BT_Y0, PA_HomeMode(BT_Y0), PA_HomeVel(BT_Y0) , ,PA_HomeOffset(BT_Y0),PA_HomeCurrentLimit(BT_Y0), ,1
...
FDEF(BT_Y0).#RL=1
FDEF(BT_Y0).#LL=1
SLLIMIT(BT_Y0)=PA_LimitN(BT_Y0)
SRLIMIT(BT_Y0)=PA_LimitP(BT_Y0)
FMASK(BT_Y0).#SRL=1
FMASK(BT_Y0).#SLL=1
